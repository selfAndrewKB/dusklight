...
void mDoExt_3DlineMat0_c::draw() {
    ...
    // buffer most recently generated by update() instead of a stale double-buffer slot.
    u8 drawBuffer = mLastUpdatedBuffer;
#else
    u8 drawBuffer = field_0x16;
#endif
    ...
    for (int i = 0; i < field_0x10; i++) {
        GXSETARRAY(GX_VA_POS, var_r28->field_0x8[drawBuffer], sizeof(cXyz) * var_r26, sizeof(cXyz), true);
        GXSETARRAY(GX_VA_NRM, var_r28->field_0x10[drawBuffer], 3 * var_r26, 3, true);
