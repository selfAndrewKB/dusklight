...
- Epona ownership is its own `horse_owner` family. Keep the authored Epona as the canonical campaign/save actor, spawn slot-assigned runtime clones for additional players, and classify horse reads as canonical story Epona, slot-assigned horse, current rider, any active horse, or horse-local collision owner before patching. A canonical horse actor can exist while vanilla presentation keeps it parked in `FLG0_NO_DRAW_WAIT`; runtime clones must mirror that native lifecycle instead of treating registration as visibility.
