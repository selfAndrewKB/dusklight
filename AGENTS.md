...
- Retained stun/grab/carry/hang effects need a retained owner slot. Use `dusk::coop::caught_stun_owner` for Gibdo-style scream stun ownership and `dusk::coop::wolf_catch_owner` for wolf-bite enemy ownership; nearby players may be affected by the same retained effect, but do not recompute the owner from nearest player, current enemy target, or damage owner after the effect begins.
...
- Flying enemies need vertical selected-target facts, not just XZ nearest-player checks. Keese proved that hover/orbit/dive positioning can stay batchable when combat targeting, retained wolf bite, and item/tool awareness are kept in their separate API families.
