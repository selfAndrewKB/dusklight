...
- Enemy-spawned weapons and child attacks should inherit the parent/master combat target when the parent owns the attack. Chilfos thrown spear proved that child projectile launch math should not resample P1; keep nontrivial target snapshots in helpers so native switch/fallthrough/goto layouts stay build-safe.
...
- Retained enemy swallow/grab/carry demos split gameplay ownership from presentation. Retain the caught slot in the matching retained-owner API, choose that slot's player camera, begin `event_presentation::Source::EnemyRetainedInteraction` only after native demo acceptance, and end it when native camera state resets or the retained owner clears. If detector and demo owner are separate actors, key the retained owner to the actor that consumes the demo/camera state.
...
- `docs/coop-singular-event-presentation-plan.md`: active singular-event presentation milestone: opt-in owner-camera fullscreen collapse, non-presenter hiding, P1/global howling stones, and captured fullscreen menu consumers.
