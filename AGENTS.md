...
- Singular authored sequences are a presentation question, not automatically an interaction-owner conversion. Keep howling stones P1/global in V1; use `event_presentation` for explicit fullscreen collapse and additional-player hiding without disabling co-op simulation.
...
- Epona ownership is its own `horse_owner` family. Keep the authored Epona as the canonical campaign/save actor, spawn slot-assigned runtime clones for additional players, and classify horse reads as canonical story Epona, slot-assigned horse, current rider, any active horse, or horse-local collision owner before patching.
...
- `docs/coop-singular-event-presentation-plan.md`: active singular-event presentation milestone: opt-in fullscreen collapse, additional-player hiding, and the P1/global howling-stone consumer.
