...
- Remind Camera-1 fullscreen edits that one visible window still needs non-native viewport world state. `shouldPresentSplitViewports()` controls second-window and split-only framebuffer work; `shouldRefreshViewportOwnedWorldState()` keeps camera-1 culling, real-shadow, kankyo/J3D material, particle-culling, and GX-light refresh active.
