...
- Remind retained enemy demo edits to keep gameplay retention and presentation collapse separate: retain the caught slot in the matching retained-owner API, key split-actor interactions to the actor that consumes demo/camera state, choose the retained slot's player camera, begin `event_presentation::Source::EnemyRetainedInteraction` after native demo acceptance, and end it when native camera state resets or the retained owner clears.
