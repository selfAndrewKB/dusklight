...
- Remind horse edits to classify canonical campaign Epona, slot-assigned runtime Epona, current rider, any-active-horse world rules, and horse-local collision ownership separately. Do not mass-replace `dComIfGp_getHorseActor()` or let runtime clones overwrite story/save state. Canonical registration does not imply visible presentation: runtime clones must mirror native parked `FLG0_NO_DRAW_WAIT` state.
...
- Remind ALINK lifecycle edits that active co-op is session intent: area loads destroy scene-local actors and camera pointers, then requested slots rebuild after the new primary ALINK is ready. Never preserve stale scene pointers.
...
- `.codex/hooks/post_tool_use_review.ps1`: adds post-edit review reminders and forces attention on `git diff --check` whitespace failures.
  ...
  - Also reminds interaction/event edits to classify singular authored sequences separately, keep howling stones P1/global, opt authored fullscreen sequences and captured menu surfaces into `event_presentation` individually, and keep fullscreen menu input ownership in `ui_owner`.
