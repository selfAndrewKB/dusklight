...
- `alink.secondary` records item ownership context for those failures: equipped item, selected item slot, item actor keep, thrown boomerang actor keep, copy-rod actor keep, ALINK item button/trigger masks, and use-button flags. These fields should stay in payload/latest and only drive JSONL events when item ownership facts actually change.
