...
- Item-specific `alink.secondary` blocks are optional. `copy_rod`, and any future `bow` or `arrow` block, should appear only when that item state is active; absence means that item did not participate in the captured state. Generic fields such as `equip_item`, `item_actor`, and `proc_name` remain always available because they describe the current ALINK state rather than one item family.
