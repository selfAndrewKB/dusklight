...
- `render.effects` records the last viewport slot/window/camera, logical viewport/scissor, native
  bloom source and composite bounds, effect-family policy, Base/Sense snapshot validity, per-slot
  Sense activation/strength/emitter ownership, per-slot Twilight camera/player/light mask, and
  current view-dependent/projected-material registrations. Exact strength and rectangles remain
  rich `latest.json` context; JSONL changes are driven by ownership, activation, policy, light-mask,
