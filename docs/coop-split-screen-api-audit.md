...
| Can one shared attention actor expose a different valid target point/state to each player? | `player_attention` owner-specific actor flags/position plus actor-local slot state | Implemented for Midna wolf-jump tags: shared tutorial/switch state remains global while traversal, readiness, native lock selection, and cursor presentation are slot-local. No-message approach ownership remains per-slot through Midna arrival and transfers only when ALINK enters the jump proc |
