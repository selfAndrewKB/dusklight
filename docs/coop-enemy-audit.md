...
| `d_a_e_sm.cpp` | `E_SM` | `@brief Chu Worm`; HIO comment `スライム - Slime`; actions: normal/move/damage/attack/core_control | retained-interaction enemy candidate | Chu Worm / Slime. Defer from light batches because hookshot carry, camera presentation, core/free state, and contact defender reads need a focused API pass rather than a targeting-only sweep. |
...
| `d_a_e_sm.cpp` | `E_SM` | Chu Worm / Slime | 3 | 2 | 0 | **Focused pass** — distance/angle in `initAction`, but hookshot carry, camera presentation, core/free state, and contact ownership make this more than a targeting-only sweep |
...
| `d_a_e_sm.cpp` | `E_SM` | 13 | retained-interaction enemy candidate | Chu Worm / スライム (Slime); normal/move/damage/attack/core_control; deferred focused pass for hookshot carry, camera presentation, core/free state, and defender/contact ownership |
