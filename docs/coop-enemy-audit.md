...
| Bomb Bug | Bomb Bug | `src/d/actor/d_a_e_bi.cpp` | `E_BI` | policy-backed targeting, first-pass validated | Wake/search, tracking, close attack gate, room sightline, home-distance checks, and the `fopAcStts_UNK_0x80000_e` collision-status band route through one Combat owner plus `selected_target_state` / `player_camera_status`; stun knockback facing uses `damage_owner`. Field testing found P2 behavior matched expectations. Hookshot/boomerang-created bomb ownership, bomb child ownership, and carry attention remain deferred item/collision surfaces. |
