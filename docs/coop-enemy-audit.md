...
| Guay | Guay | `src/d/actor/d_a_e_ge.cpp` | `E_GE` | policy-backed targeting, pending validation | Circle-flight flyer pass. Wait/fly/caw/surprise awareness, attack orbit, dive gate, attack path obstruction, shield knockback angle, and selected-slot presentation-camera angle route through one Combat owner plus `selected_target_state`; boomerang wind uses `item_awareness` so any active owner's boomerang can trigger/focus the reaction. Arrow peer-surprise scans remain vanilla projectile proximity, and no defender-owner, caught/grab, spawned-child, or broader camera/demo surfaces were found in this pass. |
...
| `d_a_e_ge.cpp` | `E_GE` | wait/fly/attack/back/caw/wind/shield actions | flying enemy candidate | Guay. Combat targeting/selected-target state, selected-slot presentation-camera angle, and boomerang item awareness are implemented pending validation. |
...
| `d_a_e_ge.cpp` | `E_GE` | 9 | flying enemy candidate | Guay; circle-flight combat targeting/selected-target state converted; selected-slot presentation-camera angle and boomerang item awareness converted pending validation |
