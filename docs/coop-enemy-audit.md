...
| `d_a_e_nz.cpp` | `E_NZ` | normal/attack/stick/damage actions | regular small enemy candidate | Ghost Rat. Targeting is compact, but attack/stick attaches to Link joints, calls Midna rat-body/heavy-state effects, and clears global stick bits. Treat as a focused retained-attach owner proof rather than a quick targeting-only batch. |
...
| `d_a_e_ph.cpp` | `E_PH` | file comment "Peahat Enemy"; appear/wait/fly/hang/damage | regular enemy candidate | Peahat. Ordinary path/flight targeting is likely compatible with selected-target state, but hookshot carry, heavy-boots hang state, and explicit P1 camera/demo paths are braided through the same action path. Convert as a focused hook/presentation batch, not as filler behind an unrelated enemy. |
...
| `d_a_e_nz.cpp` | `E_NZ` | 4 | regular small enemy candidate | Ghost Rat; compact targeting, but attack/stick joint attachment and Midna/heavy-state effects need retained-attach ownership |
...
| `d_a_e_ph.cpp` | `E_PH` | 14 | regular enemy candidate | Peahat; file comment "Peahat Enemy"; path flight, hookshot carry, heavy-boots hang state, and explicit camera/demo paths need a focused hook/presentation batch |
