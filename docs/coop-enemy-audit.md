...
| Bomb Bug | Bomb Bug | `src/d/actor/d_a_e_bi.cpp` | `E_BI` | policy-backed targeting, pending validation | Wake/search, tracking, close attack gate, and home-distance checks route through one Combat owner plus `selected_target_state`; stun knockback facing uses `damage_owner`. Hookshot/boomerang-created bomb ownership, bomb child ownership, carry attention, and `dComIfGp_checkPlayerStatus0(0, fopAcStts_UNK_0x80000_e)` remain deferred item/collision/player-status surfaces. |
| Armos Titan | Armos Titan / Idelia Statue | `src/d/actor/d_a_e_is.cpp` | `E_IS` | policy-backed targeting, pending validation | Wake/move/attack search gates, LOS, attack range, and steering route through one Combat owner plus `selected_target_state`; damage remains routed through shared `cc_at_check()`. Switch-gated trap/poweroff behavior remains vanilla/global room logic; no defender/caught/grab/spawn surfaces were found in this first pass. |
| Bombskit | Bombskit / Crazy Runner | `src/d/actor/d_a_e_cr.cpp` | `E_CR` | policy-backed targeting, pending validation | Proximity awareness and evasive chase angles route through one Combat owner plus `selected_target_state`; damage remains routed through shared `cc_at_check()`. Egg child spawning remains vanilla self-owned behavior; no defender/caught/grab/camera surfaces were found in this first pass. |
| Bari | Bari | `src/d/actor/d_a_e_dk.cpp` | `E_DK` | policy-backed targeting, pending validation | Search, chase hop direction, attack gates, first-attack cooldown distance, and selected-target electric contact state route through one Combat owner plus `selected_target_state`; damage remains routed through shared `cc_at_check()`. Body/core split appears self-contained; no defender/caught/grab/spawn/camera surfaces were found in this first pass. |
...
| `d_a_e_sm.cpp` | `E_SM` | HIO comment `スライム - Slime`; actions: normal/move/damage/attack/core_control | regular proximity candidate | Slime; contact/proximity attacker. Defer from light batches because hookshot carry, camera presentation, core/free state, and contact defender reads need a focused API pass rather than a targeting-only sweep. |
| `d_a_e_sm2.cpp` | `E_SM2` | merge/split/roof actions; shared resource structure with E_SM | regular proximity candidate | Chuchu 2 / Slime variant. Normal move awareness and action target metrics now route through Combat target policy; merge/split and roof/water special states remain vanilla/self-owned. |
...
| `d_a_e_sm.cpp` | `E_SM` | 13 | regular proximity candidate | スライム (Slime); normal/move/damage/attack/core_control; contact/proximity enemy; deferred focused pass for hookshot carry, camera presentation, core/free state, and defender/contact ownership |
| `d_a_e_sm2.cpp` | `E_SM2` | 10 | regular proximity candidate | Chuchu 2 / Slime variant; merge/split/roof actions; normal move and action metrics route through Combat target policy |
